...
fn test_build_master() {
    let master = serde_json::from_slice::<'_, JsonContent>(include_bytes!("cases/brick_master.json")).expect("Cannot parse brick_master.json");
    let lcd = serde_json::from_slice::<'_, JsonContent>(include_bytes!("cases/bricklet_lcd_128x64.json")).expect("Cannot parse bricklet_lcd_128x64.json");
    ...
    let diff = TextDiff::from_lines(
        include_str!("cases/output.rs"), generated_code.as_str());
